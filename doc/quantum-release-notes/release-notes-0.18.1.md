Quantum Core version 0.18.1 is now available from:

  <https://download.quantum.org/quantum-0.18.1/>
...
To receive security and update notifications, please subscribe to:
  ...
  <https://groups.google.com/forum/#!forum/quantum-dev>
...
`/Applications/Quantum-Qt` (on Mac) or `quantumd`/`quantum-qt` (on
Linux).
...
Quantum Core is supported and extensively tested on operating systems
using the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not
recommended to use Quantum Core on unsupported systems.

Quantum Core should also work on most other Unix-like systems but is not
as frequently tested on them.

From 0.17.0 onwards, macOS <10.10 is no longer supported. 0.17.0 is
built using Qt 5.9.x, which doesn't support versions of macOS older than
10.10. Additionally, Quantum Core does not yet change appearance when
...
- The `rpcallowip` option can no longer be used to automatically listen
  ...
  [...] -p 127.0.0.1:19243:19243` (this is an extra `:19243` over the
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](mailto:contact@quantum.org) the Quantum Core
  project to let us know about their use-case, as this feature could be
  deprecated in the future).

Wallet changes
--------------
When creating a transaction with a fee above `-maxtxfee` (default 0.1 QNTM),
the RPC commands `walletcreatefundedpsbt` and  `fundrawtransaction` will now fail
instead of rounding down the fee. Beware that the `feeRate` argument is specified
in QNTM per kilobyte, not litoshi per byte.
...
- A new [document](/blob/master/doc/quantum-conf.md)
  about the `quantum.conf` file describes how to use it to configure
  Quantum Core.

- A new document introduces Quantum Core's BIP174 [Partially-Signed
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent Quantum-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  Quantum Core to libssl vulnerabilities in the past, builders who don't
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when Quantum Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of Quantum Core, subsequently importing
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart quantumd with the `-deprecatedrpc=generate`
...
- A new `quantum-wallet` tool is now distributed alongside Quantum
...
This section describes planned changes to Quantum Core that may affect
other Quantum software and services.

- Since version 0.16.0, Quantum Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with Quantum Core 0.20 (expected about
  a year after 0.18), Quantum Core will default to native segwit
  ...
  to bech32 addresses, and if the Quantum Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in Quantum Core 0.20.
  ...
  set the `addresstype=bech32` configuration option in any Quantum Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, Quantum Core
...
- A sub-project of Quantum Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with Quantum Core.  See their [project
...
- This release changes the Random Number Generator (RNG) used from
  OpenSSL to Quantum Core's own implementation, although entropy
  gathered by Quantum Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves Quantum Core a
...
- On macOS, Quantum Core now opts out of application CPU throttling
...
- #19243 Let wallet `importmulti` RPC accept labels for standard scriptPubKeys (ryanofsky)
