Quantum Core version 0.13.2 is now available from:

  <https://download.quantum.org/quantum-0.13.2.1/>
...
ECDSA signatures inside Quantum transactions now use validation using
...
Previous versions of Quantum Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Quantum Core was to raise the default minimum
relay fee.

Quantum Core 0.13.2 will have a strict maximum size on the mempool. The
...
Quantum Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running quantumd without having to do any manual configuration.
...
Quantum Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Quantum Core will relay transactions with
...
entirely in Quantum Core 0.13, and it is at this time undecided whether the
...
Quantum Core automatically creates a hidden service to listen on, without
manual configuration. Quantum Core will also use Tor automatically to connect
...
Quantumd can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Quantum
Core to use floating fees. By default, Quantum Core will use floating
fees.
...
(default: `0.0002` QNTM/kB).

At all times, Quantum Core will cap fees at `-maxtxfee=<x>` (default:
0.10) QNTM.
Furthermore, Quantum Core will never create transactions paying less than
...
`quantum.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
Then, to tunnel a SSL connection on 219243 to a RPC server bound on localhost on port 19334 do:

    stunnel -d 219243 -r 127.0.0.1:19334 -p stunnel.pem -P ''
...
caching. A sample config for apache2 could look like:
    ...
    <Location /quantumrpc>
        ProxyPass http://127.0.0.1:19243/
        ProxyPassReverse http://127.0.0.1:19243/
        # optional enable digest auth
        # AuthType Digest
        # ...

        # optional bypass quantumd rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< quantumrpc:<password>
    </Location>
...
Restarting quantumd will no longer clear out the list of banned peers; instead
...
for a new opcode in the Quantum scripting system that in combination with
...
Quantum transactions currently may specify a locktime indicating when
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Quantum wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Quantum.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Quantum block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of quantums they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of quantums being spent is exactly
...
- **Increased security for multisig:** Quantum addresses (both P2PKH addresses
  ...
  believed to be provided by Quantum's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of Quantum's current ledger (called the UTXO set),
  ...
  note that Quantum Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Quantum users to individually opt-in to almost any change in the Quantum
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and Quantum Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the Quantum
client to Quantum Core 0.13.2 to signal for segwit support. When a
...
Since Quantum Core 0.10.0, nodes have defaulted to only relaying and
...
- `quantum-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `quantum-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
