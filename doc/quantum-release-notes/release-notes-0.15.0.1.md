Quantum Core version *0.15.0* is now available from:

  <https://download.quantum.org/quantum-0.15.0/>
...
installer (on Windows) or just copy over `/Applications/Quantum-Qt` (on Mac)
or `quantumd`/`quantum-qt` (on Linux).
...
Quantum Core is extensively tested on multiple operating systems using
...
- The `estimatesmartfee` RPC interface has been changed (See [PR 10707](https://github.com/bitcoin/bitcoin/pull/10707)):
    ...
    - If Quantum Core has not been running for long enough and has not seen enough blocks or transactions to produce an accurate fee estimation, an error will be returned (previously a value of -1 was used to indicate an error, which could be confused for a feerate).
...
Quantum Core now supports loading multiple, separate wallets (See [PR 8694](https://github.com/bitcoin/bitcoin/pull/8694), [PR 10849](https://github.com/bitcoin/bitcoin/pull/10849)). The wallets are completely separated, with individual balances, keys and received transactions.

Multi-wallet is enabled by using more than one `-wallet` argument when starting Quantum, either on the command line or in the Quantum config file.
...
Quantum Core 0.15.0 contains the following changes to the RPC interface and `quantum-cli` for multi-wallet:

* When running Quantum Core with a single wallet, there are **no** changes to the RPC interface or `quantum-cli`. All RPC calls and `quantum-cli` commands continue to work as before.
* When running Quantum Core with multi-wallet, all *node-level* RPC methods continue to work as before. HTTP RPC requests should be send to the normal `<RPC IP address>:<RPC port>/` endpoint, and `quantum-cli` commands should be run as before. A *node-level* RPC method is any method which does not require access to the wallet.
* When running Quantum Core with multi-wallet, *wallet-level* RPC methods must specify the wallet for which they're intended in every request. HTTP RPC requests should be send to the `<RPC IP address>:<RPC port>/wallet/<wallet name>/` endpoint, for example `127.0.0.1:19243/wallet/wallet1.dat/`. `quantum-cli` commands should be run with a `-rpcwallet` option, for example `quantum-cli -rpcwallet=wallet1.dat getbalance`.
...
- When using Quantum Core in multi-wallet mode, RPC requests for wallet methods must specify
...
- #19244 `fa625b0` Document CWalletTx::mapValue entries and remove erase of nonexistent "version" entry (ryanofsky)
