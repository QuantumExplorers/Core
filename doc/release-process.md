...
Check out the source code in the following directory hierarchy.
    ...
    git clone https://github.com/quantum-project/gitian.sigs.qntm.git
    git clone -detached-sigs.git
    git clone https://github.com/devrandom/gitian-builder.git
    git clone .git

### Quantum maintainers/release engineers, suggestion for writing release notes
...
Ensure your gitian.sigs.qntm are up-to-date if you wish to gverify your builds against other Gitian signatures.

    pushd ./gitian.sigs.qntm
    git pull
    popd
...
By default, Gitian will fetch source files as needed. To cache them ahead of time, make sure you have checked out the tag you want to build in quantum, then:
...
### Build and sign Quantum Core for Linux, Windows, and macOS:

    export GITIAN_THREADS=2
    export GITIAN_MEMORY=3000
    
    pushd ./gitian-builder
    ./bin/gbuild --num-make $GITIAN_THREADS --memory $GITIAN_MEMORY --commit quantum=v${VERSION} ../quantum/contrib/gitian-descriptors/gitian-linux.yml
    ./bin/gsign --signer "$SIGNER" --release ${VERSION}-linux --destination ../gitian.sigs.qntm/ ../quantum/contrib/gitian-descriptors/gitian-linux.yml
    mv build/out/quantum-*.tar.gz build/out/src/quantum-*.tar.gz ../

    ./bin/gbuild --num-make $GITIAN_THREADS --memory $GITIAN_MEMORY --commit quantum=v${VERSION} ../quantum/contrib/gitian-descriptors/gitian-win.yml
    ./bin/gsign --signer "$SIGNER" --release ${VERSION}-win-unsigned --destination ../gitian.sigs.qntm/ ../quantum/contrib/gitian-descriptors/gitian-win.yml
    ...
    ./bin/gbuild --num-make $GITIAN_THREADS --memory $GITIAN_MEMORY --commit quantum=v${VERSION} ../quantum/contrib/gitian-descriptors/gitian-osx.yml
    ./bin/gsign --signer "$SIGNER" --release ${VERSION}-osx-unsigned --destination ../gitian.sigs.qntm/ ../quantum/contrib/gitian-descriptors/gitian-osx.yml
...
Build output expected:
  ...
  4. macOS unsigned installer and dist tarball (`quantum-${VERSION}-osx-unsigned.dmg`, `quantum-${VERSION}-osx64.tar.gz`)
  5. Gitian signatures (in `gitian.sigs.qntm/${VERSION}-<linux|{win,osx}-unsigned>/(your Gitian key)/`)

### Verify other gitian builders signatures to your own. (Optional)

Add other gitian builders keys to your gpg keyring, and/or refresh keys: See `../quantum/contrib/gitian-keys/README.md`.

Verify the signatures

    pushd ./gitian-builder
    ./bin/gverify -v -d ../gitian.sigs.qntm/ -r ${VERSION}-linux ../quantum/contrib/gitian-descriptors/gitian-linux.yml
    ./bin/gverify -v -d ../gitian.sigs.qntm/ -r ${VERSION}-win-unsigned ../quantum/contrib/gitian-descriptors/gitian-win.yml
    ./bin/gverify -v -d ../gitian.sigs.qntm/ -r ${VERSION}-osx-unsigned ../quantum/contrib/gitian-descriptors/gitian-osx.yml
    popd

### Next steps:

Commit your signature to gitian.sigs.qntm:
...
Codesigner only: Sign the macOS binary:
    ...
    transfer quantum-osx-unsigned.tar.gz to macOS for signing
    tar xf quantum-osx-unsigned.tar.gz
    ...
    $   xcrun altool --notarize-app --primary-bundle-id "org.quantum.Quantum-Qt" -u "<apple-id-email>" -p "@keychain:<apple-id-notarisation-app-specific-password>" --asc-provider <team-id-shortcode> -t osx -f quantum-${VERSION}-osx.dmg
    ...
    $   xcrun stapler staple dist/Quantum-Qt.app

Codesigner only: Sign the windows binaries:

    tar xf quantum-win-unsigned.tar.gz
...
Codesigner only: Commit the detached codesign payloads:
    ...
    cd ~/quantum-detached-sigs
    #checkout the appropriate branch for this release series
    rm -rf *
    tar xf signature-osx.tar.gz
    tar xf signature-win.tar.gz
    #copy the notarization ticket to detached-sigs repo
    cp dist/Quantum-Qt.app/Contents/CodeResources osx/dist/Quantum-Qt.app/Contents/
    git add -A
...
- Detached signatures will then be committed to the [quantum-detached-sigs](-detached-sigs) repository, which can be combined with the unsigned apps to create signed binaries.

Create (and optionally verify) the signed macOS binary:

    pushd ./gitian-builder
    ./bin/gbuild -i --commit signature=v${VERSION} ../quantum/contrib/gitian-descriptors/gitian-osx-signer.yml
    ./bin/gsign --signer "$SIGNER" --release ${VERSION}-osx-signed --destination ../gitian.sigs.qntm/ ../quantum/contrib/gitian-descriptors/gitian-osx-signer.yml
    ./bin/gverify -v -d ../gitian.sigs.qntm/ -r ${VERSION}-osx-signed ../quantum/contrib/gitian-descriptors/gitian-osx-signer.yml
...
Commit your signature for the signed macOS/Windows binaries:
    ...
    git push  # Assuming you can push to the gitian.sigs.qntm tree
    popd
...
quantum-${VERSION}-riscv64-linux-gnu.tar.gz
...
- Announce the release:
  ...
  - quantum-dev mailing list

  - blog.quantum.org blog post

  - Update title of #quantum and #quantum-dev on Freenode IRC
