...
- [Development guidelines](#development-guidelines)
    - [General Quantum Core](#general-quantum-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Quantum Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Quantum Core is a multi-threaded application, and deadlocks or other
...
```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_quantum
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_quantum --log_level=test_suite
$ valgrind -v --leak-check=full src/quantumd -printtoconsole
...
# A coverage report will now be accessible at `./test_quantum.coverage/index.html`.
```
...
To profile a running quantumd process for 60 seconds, you could use an
invocation of `perf record` like this:

```sh
$ perf record \
    -g --call-graph dwarf --per-thread -F 140 \
    -p `pgrep quantumd` -- sleep 60
```
...
Quantum Core can be compiled with various "sanitizers" enabled, which add
...
Some of these are maintained by active developers of Quantum Core, in which case changes should probably go
directly upstream without being PRed directly against the project. They will be merged back in the next
subtree merge.

Others are external projects without a tight relationship with our project. Changes to these should also
be sent upstream, but bugfixes may also be prudent to PR against Quantum Core so that they can be integrated
...
file descriptors, it will cause problems with Quantum's `select()` loop, because
...
compatibility between nodes. This happened in Quantum 0.8 (when LevelDB was
...
would be to revert the upstream fix before applying the updates to Quantum's
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `quantum-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `quantum-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of Quantum Core, the wallet was always
