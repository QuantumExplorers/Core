    ...
    const bool sign)
{
    ...
    while (true) {
        ...
        // the recipients are funded through the pegout kernel instead of traditional QNTM outputs.
        ...
        // Calculate transaction size and the total necessary fee amount (includes QNTM and MWEB fees)
    ...
    if (new_tx.mweb_type != MWEB::TxType::QNTM_TO_QNTM) {
...
bool TxAssembler::AttemptCoinSelection(InProcessTx& new_tx, const CAmount& nTargetValue) const
{
    ...
    if (new_tx.recipients.front().IsMWEB()) {
        ...
        if (SelectCoins(new_tx, nTargetValue, params_pegin)) {
            return std::any_of(new_tx.selected_coins.cbegin(), new_tx.selected_coins.cend(), is_qntm);
        }
    } else {
        // First try to construct a QNTM-to-QNTM transaction
        CoinSelectionParams mweb_to_mweb = new_tx.coin_selection_params;
        mweb_to_mweb.input_preference = InputPreference::QNTM_ONLY;
        ...
        // If QNTM-to-QNTM fails, create a peg-out transaction
